"""B 端演示种子（商品/SKU 矩阵、双仓库存、三状态订单、待审改价、回溯订单）

链路：app.db.seed.seed_on_startup() → ensure_b2b_demo()（基线 B 端演示）；
     app.db.seed.ensure_closed_loop_demo() → _seed_backfill_orders()（大屏 GMV 趋势分桶）。
对齐数据模型与存储设计.md §6 迁移节；由 B2B_SEED_DEMO 控制，已存在商品即跳过。
"""
...
# 回溯订单：近 6 日每天 3 单，让大屏「近 7 日 GMV 趋势」与今日/昨日涨跌有真实分桶
_BACKFILL_DAYS = 6
_ORDERS_PER_DAY = 3
...
async def _seed_backfill_orders(
    ...
    for day_offset in range(_BACKFILL_DAYS, 0, -1):
        ...
        for index in range(_ORDERS_PER_DAY):
            picks = [
                skus[(day_offset + index) % len(skus)],
                skus[(day_offset + index + 3) % len(skus)],
            ]
            items: list[dict[str, object]] = []
            total = 0
            for pick in picks:
                qty = 1 + ((day_offset + index) % 3)
                spu_no = spu_of.get(pick.product_id, "")
                items.append(
                    {
                        "sku_id": pick.id,
                        "sku_code": "-".join(p for p in (spu_no, pick.color, pick.size) if p),
                        "color": pick.color,
                        "size": pick.size,
                        "qty": qty,
                        "price": pick.sale_price,
                    }
                )
                total += pick.sale_price * qty
            stamp = day.strftime("%Y%m%d")
            order = SalesOrder(
                tenant=tenant,
                platform=("taobao", "douyin", "weixin")[index % 3],
                outer_id=f"BF{stamp}{index + 1:02d}",
                items=json.dumps(items, ensure_ascii=False),
                total=total,
                status="completed" if index != 0 else "shipped",
                trace_id=f"bf{stamp}{index + 1:02d}",
                created_at=day,
            )
            db.add(order)
