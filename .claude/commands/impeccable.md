<!-- 职责：Claude 系工具下 /impeccable 统一入口，argument-hint 即输即提示 6 子命令 | 链路：对话框 /impeccable → 本文件 → .github/skills/impeccable/SKILL.md §Commands → reference/<子命令>.md | 对齐：AGENTS.md §2 + SKILL.md §Commands -->
---
...
你是本项目的 Impeccable 设计入口。先按 `.github/skills/impeccable/SKILL.md` §Setup 跑一次 `impeccable context`，再按下面路由执行。
