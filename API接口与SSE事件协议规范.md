...
- `POST /workbench/sessions/{id}/claim` → `ok(队列行)`，`pending|none → handling` + `assignee=Token 用户名`；已被他人认领 → `1001`「已被 XX 接管，转为只读围观」（前端据此切只读态，不弹系统错误）。抢接为**原子比较交换**（条件 UPDATE + rowcount）：两坐席同抢只有一人成功，输家必拿 1001，不会互相覆盖 `assignee`；本人已认领的会话重复认领幂等刷新。
