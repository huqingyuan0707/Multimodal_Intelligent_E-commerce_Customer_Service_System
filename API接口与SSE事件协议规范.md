...
- 采购（**已实现**）：`GET /suppliers?keyword=&page=&size=`（`{items[{id,name,pay_terms,pass_rate,created_at}],total,page,size}`，pass_rate 为 0~1 浮点，展示由前端格式化）、`POST /suppliers {name,pay_terms?,pass_rate?}`（名称必填、合格率须 0~1 否则 `1001`，租户内不去重）、`GET /purchase?status=&page=&size=`（`{items,total,page,size}`，`status` 精确筛选，非法值 `1001`）、`POST /purchase {supplier_id,warehouse_id?,items[{sku_id,qty,price}],eta?}`（恒为 `draft`；`eta` 须 `YYYY-MM-DD`；SKU/收货仓跨租户 404、数量须 1~100000 且明细 ≤50 条否则 `1001`；**行名/颜色/尺码由服务端按 SKU 快照回填，不接受前端传入**）、`POST /purchase/{id}/approve {approved,reason?}`（仅 `draft` 可审否则 `3005`；`approved=false` 即驳回且**理由必填**，驳回为终态）、`POST /purchase/{id}/receive {eta?}`（仅 `approved` 可登记到货否则 `3005`）、`POST /purchase/{id}/qc {passed,note}`（仅 `received` 可质检否则 `3005`，`note` 必填；`passed=true` 写 `inventory` + `stock_moves` 并置 `stocked`，未指定收货仓 `1001`；`passed=false` 置 `returned` **不入库**）。单行回 `{id,supplier_id,supplier_name,warehouse_id,items,qty_total,amount,status,status_label,allowed_actions,eta,qc_result,qc_note,created_at}`（`amount` 为分；`allowed_actions` 让前端置灰按钮）。权限 `purchase:read|write`（写 `purchase:write`），全程记 `purchase.create/approve|reject|receive|qc` 审计。
- 财务（**已实现**）：`GET /finance/bills?biz_date=&page=&size=` → `{items[{id,biz_date,receivable,received,refund,fee,freight,expected,diff,diff_warn,settled_by,settled,created_at}],total,page,size,diff_warn_cents,unsettled}`（金额一律分；差异公式唯一 `expected = receivable - refund - fee + freight`、`diff = received - expected`；`|diff| > Settings.FINANCE_DIFF_WARN_CENTS`（默认 5000 分）即 `diff_warn=true` 红字，**阈值随列表下发，前端禁硬编码**；`biz_date` 格式 `YYYY-MM-DD` 否则 `1001`）、`POST /finance/settle {biz_date}`（未结则落 `settled_by` 并记 `finance.settle` 审计；**重复确认 `1001`**，账单不存在 404；财务双人复核为 P2 预留位）。权限 `finance:read|write`（写 `finance:write`）。
...
- B端单据写操作必须带 `Idempotency-Key`；采购/调拨/报损/超阈值退款恒进审批流（**例外**：采购审批走采购单自身状态机 `POST /purchase/{id}/approve`，不入 §4.5 审批中心）。
...
- 风控（**已实现**）：`GET /risk/events?status=&kind=&page=&size=` → `{items[{id,user_ref,kind,kind_label,detail,status,status_label,reviewable,reviewer,reason,created_at}],total,page,size,pending}`（`status ∈ pending|passed|blocked` 精确筛选，非法 `1001`；`detail` 为关联图谱摘要对象，解析失败回 `{}` 不炸列表）、`POST /risk/{event_id}/pass {reason?}`、`POST /risk/{event_id}/block {reason}`（仅 `pending` 可复核否则 `3005`；**拦截理由必填**否则 `1001`；放行/拦截均落 `reviewer` 与审计 `risk.pass`/`risk.block`）。权限 `risk:review`（读亦放行 `risk:read`）。**红线**：只做人工复核留痕，**不做全自动封号**（不写 `users.status`）；`3007 RISK_BLOCKED` 是**业务动作侧**错误码（下单/退款命中拦截时返回），业务动作侧的强制拦截待 P2 接入，勿读作已达成。工单协同复用既有 `POST /tickets`（见下行）。
...
- 消息模板与到达率：`GET /admin/notify/templates?tenant=&channel=&status=&page=&size=`（含累计到达口径与 `window_note`）、`POST /admin/notify/templates`、`PUT /admin/notify/templates/{id}`（全量覆盖；同名撞他表 `1001`）、`POST /admin/notify/templates/{id}/status {status}`（**正文为空不允许启用**）、`DELETE /admin/notify/templates/{id}`、`POST /admin/notify/send {tenant,name,user_ref}`（频控键 `rl:{tenant}:notify:{user_ref}`，窗口/上限走 `Settings.NOTIFY_RATE_*`，超限 `1006`/HTTP 429；**网关未接入时如实回 `delivered=false, degraded=true, reason`，并把该次计为未送达**）、`GET /admin/notify/reach?tenant=` → `{items,total_sent,total_failed,total_delivered,reach_rate,no_data,window_note,gateway_note}`（累计口径，无时间窗）。
