...
> **⚠️ 本节为对外「目标承诺」，非「达成声明」。** 逐条「实测达标 / 代理达标 / 未实测 / 口径不符」对账以 [`执行步骤.md`](执行步骤.md)「〇、愿景 vs 实测对照」为**唯一事实源**：当前 19 项量化目标中仅 6 项 🟢 实测达标，性能 / 成本 / 真机视觉 / 灾备四整列 ⚪ 未实测。**对外引用本表任一数字前，请先核对〇节。**
...
| 多 Agent 架构 / 落地验收      | [`多模态智能电商客服系统需求文档-FRDv2.md`](多模态智能电商客服系统需求文档-FRDv2.md) §11（拆分版快照 `多Agent系统架构与落地需求.md` 保留备用）                                                               |
| **人 + AI 统一执行入口**     | [`AGENTS.md`](AGENTS.md)                                                                                                                   |

> **口径注意：** `多模态智能电商客服系统需求文档-FRDv2.md` 已升至 **v2.4**（2026-09-18，新增 §11 多 Agent 系统架构与落地需求，合并自 `多Agent系统架构与落地需求.md`），`电商开发文档.md` 自述为 **FRD v3.0**（替代原架构指南版），两者存在版本交叠；`AGENTS.md` §1 目前把「需求 / 验收口径」指向 FRDv2。**SSE 事件协议冲突已收敛（2026-09-16）**：FRDv2 FR-1.1、FRD v3 FR-1.1 与《前端工程化.md》§四/§六 均已对齐为权威口径 `source/phase/message/done`（工具调用经 `done.tool_calls[]` 透出，不是独立事件；旧 `text/tool_call/tool_result` 联合作废）。**事件形态与错误码一律以 `API接口与SSE事件协议规范.md` §5/§2 为准**；v2/v3 余下的编号交叠仍待收敛为唯一基线。
...
├── README.md
│
├── 多模态智能电商客服系统需求文档-FRDv2.md   # 需求 / 验收口径（v2.4 含 §11 多 Agent 架构）
├── 多Agent系统架构与落地需求.md              # §11 拆分版快照（归档备用）
...
> 以下为**发布门禁**，勾选框**当前均未达**（逐项现状见 [`执行步骤.md`](执行步骤.md)〇节）；任一未达禁发布。
...
> 本表 ✅/🔶 记的是**代码落地度**（链路通没通、有无测试兜底），**不等于** §1 业务指标达标；指标达标度唯一事实源见 [`执行步骤.md`](执行步骤.md)〇节。
...
| `backend/` 应用代码                                                               | ✅ 已落地：鉴权（JWT + 用户切换）、SSE 流式对话 + RAG、订单 / 商品 / 库存 / 物流 / 评价 / 营销 / 文档 / 审批 / 任务 / 工单 / 管理中心、坐席工作台（`handoff_status` 流转 + 转人工触发规则表 `handoff_rules` 9 条判据，v0.2.9），种子数据 + Alembic 迁移，覆盖率门禁 ≥ 70% 达标 |
...
> 如需换用 Apache-2.0（含显式专利授权，企业/模型类项目更稳），改 `LICENSE` 文件并同步本节；版权主体名以实际法律实体为准。
