...
> **口径注意：** `多模态智能电商客服系统需求文档-FRDv2.md` 自述为 **FRD v2.0**，`电商开发文档.md` 自述为 **FRD v3.0**（替代原架构指南版），两者存在版本交叠；`AGENTS.md` §1 目前把「需求 / 验收口径」指向 FRDv2。**SSE 事件协议冲突已收敛（2026-09-16）**：FRDv2 FR-1.1、FRD v3 FR-1.1 与《前端工程化.md》§四/§六 均已对齐为权威口径 `source/phase/message/done`（工具调用经 `done.tool_calls[]` 透出，不是独立事件；旧 `text/tool_call/tool_result` 联合作废）。**事件形态与错误码一律以 `API接口与SSE事件协议规范.md` §5/§2 为准**；v2/v3 余下的编号交叠仍待收敛为唯一基线。
