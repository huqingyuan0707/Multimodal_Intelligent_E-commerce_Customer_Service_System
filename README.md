...
> 版本：v0.3.5 | 日期：2026-09-16 | 状态：**P0/P1 代码已落地，CI/CD 与容器化部署已跑通**（文档基线 → 可运行系统）
...
| `backend/` 应用代码                                                               | ✅ 已落地：鉴权（JWT + 用户切换）、SSE 流式对话 + RAG、订单 / 商品 / 库存 / 物流 / 评价 / 营销 / 文档 / 审批 / 任务 / 工单 / 管理中心、坐席工作台（`handoff_status` 流转 + 转人工触发规则表 `handoff_rules` 9 条判据 + 技能组路由/智能分配/负载面板 `handoff_routing`，v0.3.5），种子数据 + Alembic 迁移，覆盖率门禁 ≥ 70% 达标 |
| `frontend/` 应用代码                                                              | ✅ 已落地：14 个域页面（登录 / 对话 / 坐席工作台 / 管理 / 看板 / 商品 / 库存 / 订单…），`v-permission` 按钮级权限、mock 降级、列表分页规范；对话页渲染 `done.tool_calls` 工具调用卡（v0.2.8）；工作台队列技能组页签/排队位播报/智能分配按钮（v0.3.5）；**双主题**：浅色默认 + 深色可切（`useTheme` 持久化，挂墙大屏 `/screen` 锁深色，v0.3.0）；全站统一阿里巴巴普惠体 3.0（本地字面优先，v0.2.10）                     |
...
| 黄金集（≥ 500 条）与评估流水线                                                    | 🔶 可观测地基已落（v0.3.1）：`observability.record()` 真留痕（内存计数 + JSONL `data/observability/`），`GET /workbench/metrics` 出 30s 接起率/LLM·工具成功率；余：黄金集、LLM-as-judge、Prometheus/Langfuse 网关 |
